...
public class RSA {
	...
	public static Map<String, String> generateKeyPair() throws Exception {
		...
		String pub = new String(Base64.encodeBase64(publicKeyBytes),
				EncryptAndDecryptConfig.CHAR_ENCODING);
		/** 得到私钥 */
	...
	public static String encrypt(String source, String publicKey)
		...
		Cipher cipher = Cipher.getInstance(EncryptAndDecryptConfig.RSA_ALGORITHM);
	...
	public static String sign(String content, String privateKey) {
		String charset = EncryptAndDecryptConfig.CHAR_ENCODING;
		try {
	 ...
	 * @Return:
	 **/
	public static void createKeyAndSavePath(String savePath, List<String> fileNames) {
		try {
			Map<String, String> keyMap = generateKeyPair();
			for (String fileName : fileNames) {
				if (fileName.contains("public")) {
					FileUtils.writeFileFromString(savePath + "/" +fileName ,keyMap.get("publicKey"),false);
				}
				if (fileName.contains("private")) {
					FileUtils.writeFileFromString(savePath + "/" +fileName ,keyMap.get("privateKey"),false);
				}
			}
		} catch (Exception e) {
			log.error("生成密钥失败{}{}",savePath,fileNames, e);
			throw new RuntimeException("生成密钥失败", e);
		}
	}
	public static String readKeyFromPath(String readPath, String fileName) {
		String keyContent = "";
		try {
			keyContent = FileUtils.readFile2String(readPath + "/" +fileName ,"UTF-8");
		} catch (Exception e) {
			log.error("读取密钥失败{}{}",readPath,fileName, e);
